...
public final class BotPathfinder {
    ...
    public static List<Move> findPathMoves(
        ...
        int nodeLimit = opts.anyEnabled() ? Config.pathfindingMaxNodesExtended() : Config.pathfindingMaxNodes();
        ...
        while (!open.isEmpty() && explored++ < nodeLimit) {
            ...
            for (int[] nb :
                    neighbors(world, cur.pos().x(), cur.pos().y(), cur.pos().z(), opts)) {
    ...
    public static List<Pos> findPath(World world, int sx, int sy, int sz, int tx, int ty, int tz, PathOptions opts) {
    ...
    private static boolean isOpenable(World world, int x, int y, int z) {
        ...
        try {
            ...
            Block block = world.getBlockAt(x, y, z);
            if (block.getBlockData() instanceof Door door) {
                Block bottom = door.getHalf() == org.bukkit.block.data.Bisected.Half.BOTTOM
                        ? block
