...
- **Single action per bot.** Each user task (`move`/`find`/`left-click`/`right-click`/`attack`) calls `FakePlayerManager.beginExclusiveAction(uuid, keep)` as it commits, which stops the bot's other tasks. Background PVE checks `hasActiveManualAction(uuid)` and yields. Do not reintroduce concurrent tasks / multitasking.
- **Pause/resume.** Every per-tick action loop (pathfinding movement, find/mine, left-click, right-click, attack, PVE) early-returns on `FakePlayer#isActionsPaused()` **without** clearing its state, so the task resumes when the flag clears. Auto-eat sets/clears this flag. Keep new action loops honoring it.
- **Click aim precision.** `left-click`/`right-click` aim at the sender's exact ray-trace hit position (`getHitPosition()`), not the block-face centre, and resolve an out-of-reach stand spot via `resolveStandLocation` (prefer the sender's own location). Do **not** re-add an "attached interactive block" redirect (the old `checkForAttachedInteractiveBlock`): the interaction ray trace uses the block OUTLINE shape (`ignorePassableBlocks=false`), so a button/lever is only triggered when the bot actually aims at its hit box.
